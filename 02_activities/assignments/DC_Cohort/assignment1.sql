...
SELECT
  ...
  SUM(COALESCE(cp.quantity,0) * COALESCE(cp.cost_to_customer_per_qty,0)) AS total_spent
FROM customer AS c
...
HAVING
  SUM(COALESCE(cp.quantity,0) * COALESCE(cp.cost_to_customer_per_qty,0)) > 2000
ORDER BY
